...
if($user->Authentication() && !empty($_POST['order_id']) && !empty($_POST['address']) && !empty($_POST['to_bank'])){
    ...
    echo "Var: ".$_SERVER['HTTP_X_REQUESTED_WITH'].'<br>';

    // if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){

    if(isset($_POST)){

        echo'Found post request!<br>';
        echo "isset image_file: <b>".(isset($_FILES['image_file'])?'true':'false').'</b><br><br>';
        echo "is_uploaded_file image_file: <b>".(is_uploaded_file($_FILES['image_file']['tmp_name'])?'true':'false').'</b><br>';
        echo "realpath: ".realpath($_FILES['image_file']['tmp_name']).'<br>';
        ...
        switch( $_FILES['image_file']['error'] ) {
            ...
            case UPLOAD_ERR_FORM_SIZE:
                $message .= ' - file too large (limit of '.getMaxUploadLimit().' MB).';
                break;
            ...
            case UPLOAD_ERR_NO_FILE:
                ...
                break;
            default:
                $message .= ' - internal error #'.$_FILES['image_file']['error'];
                break;
        }
        ...
        echo'<hr>';
        echo'<b>Option config:</b><br>';
        echo (int)(ini_get('upload_max_filesize')).' MB upload_max_filesize<br>';
        echo (int)(ini_get('post_max_size')).' MB post_max_size<br>';;
        echo (int)(ini_get('memory_limit')).' MB memory_limit<br><br>';;
